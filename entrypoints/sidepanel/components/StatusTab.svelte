<script lang="ts">
  ...
  $: {
    // 状況が更新された時の処理
    if (statuses.length > 0 && lastUpdateTime !== lastScan && lastScan > 0) {
  ...
  function startAutoRefresh() {
    ...
    refreshInterval = setInterval(
      async () => {
        if (isActive && watchedNames.length > 0 && !isRefreshing) {
          try {
            isRefreshing = true;
      ...
      Math.max($settings?.refreshInterval || 1000, 500),
    ); // 最小500ms間隔
  }
  ...
  onDestroy(() => {
    console.log("StatusTab destroyed");
    stopAutoRefresh();
  });
</script>
