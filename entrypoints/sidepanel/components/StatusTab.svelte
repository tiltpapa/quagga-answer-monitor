<script lang="ts">
  ...
  import { monitorState, settings, isLoading, error, appStore } from '../stores/appStore.js';
  ...
  // 監視対象と状況のマッピング
  $: statusMap = new Map(statuses.map(status => [status.watchedNameId, status]));
  ...
  $: {
    ...
    if (statuses.length > 0 && lastUpdateTime !== lastScan) {
      ...
      statuses.forEach(status => {
        if (status.lastUpdated > Date.now() - 2000) { // 2秒以内の更新
  ...
  function startAutoRefresh() {
    ...
    refreshInterval = setInterval(async () => {
      if (isActive && watchedNames.length > 0) {
        try {
          // 状態を強制的に更新
          const newState = await appStore.getStatus();
          // ストアは自動的に更新される
        } catch (error) {
  ...
  {#if lastScan > 0}
    <div class="text-xs text-gray-500 text-center bg-gray-50 rounded p-2">
      <div class="flex items-center justify-center space-x-2">
        <span>🕐</span>
        <span>最終スキャン: {formatTime(lastScan)} ({getRelativeTime(lastScan)})</span>
      </div>
      {#if displayItems.filter(item => !item.found).length > 0}
        <div class="mt-1 text-gray-400">
          {displayItems.filter(item => !item.found).length}件の監視対象が見つかりませんでした
        </div>
      {/if}
    </div>
  ...
  {:else}
    ...
    <div class="space-y-3">
      {#each displayItems as item (item.watchedName.id)}
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm transition-all duration-300 {updatedItems.has(item.watchedName.id) ? 'ring-2 ring-blue-300 bg-blue-50' : ''}">
          <!-- 監視対象名前 -->
          <div class="flex items-center justify-between mb-2">
            <div class="flex items-center space-x-2">
              <span class="font-medium text-gray-800">{item.watchedName.name}</span>
              {#if item.watchedName.exactMatch}
                <span class="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded">完全一致</span>
              {:else}
                <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">部分一致</span>
              {/if}
            </div>
          </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <!-- 状況アイコンとテキスト -->
              <div class="flex items-center space-x-2">
                <span class="text-xl">{getRightStatusIcon(item.hasRight, item.found)}</span>
                <span class="text-sm font-medium {getRightStatusClass(item.hasRight, item.found).split(' ')[0]}">
          ...
          {#if !item.found && lastScan > 0}
            <div class="mt-2 p-2 bg-gray-50 rounded text-xs text-gray-600">
              ...
              <div class="mt-1 text-gray-500">
                {#if item.watchedName.exactMatch}
                  「{item.watchedName.name}」と完全一致する名前が見つかりませんでした
                {:else}
          ...
          {#if item.found && item.matchedName && item.matchedName !== item.watchedName.name}
            <div class="mt-2 text-xs text-gray-600">
              マッチした名前: <span class="font-mono bg-gray-100 px-1 rounded">{item.matchedName}</span>
            </div>
          {/if}

          <!-- 状況バッジ -->
          <div class="mt-3">
            <div class="inline-flex items-center px-2 py-1 rounded-full text-xs {getRightStatusClass(item.hasRight, item.found)}">
              {getRightStatusText(item.hasRight, item.found)}
            </div>
          </div>
        </div>
    ...
    <div class="mt-6 bg-gray-50 rounded-lg p-3">
      <div class="grid grid-cols-4 gap-3 text-center">
        <div>
          <div class="text-lg font-semibold text-gray-800">{displayItems.length}</div>
